"""Handle processing images to detect fighting."""
...
class FightdetectionRealTimeProcessor(RealTimeProcessorApi):
    def __init__(
        self,
        ...
        self.detected_fight: dict[str, float] = {}
        #self.labelmap: dict[int, str] = {}
        self.frame_buffer = []
        # Load model and label paths from the configuration
        self.model_path = config.classification.fight_det.fighting_model_path
        

        print('fighting model path', self.model_path)
    ...
    def process_frame(self, obj_data, frame):
        ...
        print('inside process_frame fighting det')
    ...
    def expire_object(self, object_id):
        if object_id in self.detected_fight:
            self.detected_fight.pop(object_id)
