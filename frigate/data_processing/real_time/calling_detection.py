"""Handle processing images to detect calling."""
...
class CalldetectionRealTimeProcessor(RealTimeProcessorApi):
    def __init__(
        self,
        ...
        self.detected_calling: dict[str, float] = {}
        # self.labelmap: dict[int, str] = {}

        # Load model and label paths from the configuration
        self.model_path = config.classification.call_det.calling_model_path
        # self.label_path = config.classification.vehicle_attr.vehicle_attr_label_path

        print('Calling model path', self.model_path)
    ...
    def process_frame(self, obj_data, frame):
        ...
        print('inside process_frame calling detection')
    ...
    def expire_object(self, object_id):
        if object_id in self.detected_calling:
            self.detected_calling.pop(object_id)
